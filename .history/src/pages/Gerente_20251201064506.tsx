  ...
  const [inventario, setInventario] = useState<{ nombre?: string; name?: string; cantidad?: number; unidad?: string }[]>([]);
          ...
          {showInventarioModal && (
            <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-xl relative">
                <button className="absolute top-4 right-4 btn btn-sm btn-error" onClick={() => setShowInventarioModal(false)}>Cerrar</button>
                <h2 className="text-2xl font-bold mb-4 text-primary">Inventario en tiempo real</h2>
                {inventario.length === 0 ? (
                    ...
                    <tbody>
                      {inventario.map((item, idx) => (
                        <tr key={idx} className="border-b">
                          <td className="px-4 py-2">{item?.nombre ?? item?.name ?? '-'}</td>
