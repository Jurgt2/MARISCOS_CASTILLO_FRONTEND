...
export default function Inventario() {
  ...
  const handleEtiquetasQR = async () => {
    ...
    for (const [index, producto] of productos.entries()) {
      try {
        const url = await QRCode.toDataURL(producto.id);
        doc.text(`Producto: ${producto.nombre}`, 10, 10 + index * 50);
        doc.text(`ID: ${producto.id}`, 10, 20 + index * 50);
        doc.addImage(url, 'PNG', 10, 30 + index * 50, 50, 50);
        ...
        alert(`Error generando QR para el producto con ID ${producto.id}`);
        return;
      }
    }

    doc.save('EtiquetasQR.pdf');
    alert('PDF con etiquetas QR generado exitosamente.');
  };
