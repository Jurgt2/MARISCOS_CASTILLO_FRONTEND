                    ...
                    <ProductGridLite
                      ...
                      onAddProduct={(
                        ...
                        if (selectedTables.length === 1) {
                          ...
                          // Buscar el stock por coincidencia de nombre (busca en todas las keys del inventario)
                          let stockActual = 0;
                          let claveInventario = nombreProducto;
                          
                          // Buscar coincidencia exacta primero
                          if (platillosInventario[nombreProducto]) {
                            stockActual = platillosInventario[nombreProducto];
                            claveInventario = nombreProducto;
                          } else {
                            // Buscar coincidencia parcial (ignora mayúsculas/minúsculas y busca palabras clave)
                            const nombreNormalizado = nombreProducto.toLowerCase();
                            
                            for (const [key, value] of Object.entries(platillosInventario)) {
                              const keyNormalizada = key.toLowerCase();
                              
                              // Buscar si alguna parte del nombre coincide
                              if (
                                keyNormalizada.includes(nombreNormalizado) || 
                                nombreNormalizado.includes(keyNormalizada) ||
                                // Buscar por palabras clave específicas
                                (nombreNormalizado.includes('chile limón') && keyNormalizada.includes('chile limón')) ||
                                (nombreNormalizado.includes('al mojo de ajo') && keyNormalizada.includes('mojo de ajo')) ||
                                (nombreNormalizado.includes('española') && keyNormalizada.includes('española')) ||
                                (nombreNormalizado.includes('diabla') && keyNormalizada.includes('diabla')) ||
                                (nombreNormalizado.includes('empanizados') && keyNormalizada.includes('empanizados')) ||
                                (nombreNormalizado.includes('coco') && keyNormalizada.includes('coco'))
                              ) {
                                stockActual = value as number;
                                claveInventario = key;
                                console.log(`✅ Coincidencia encontrada: "${nombreProducto}" -> "${key}" (Stock: ${value})`);
                                break;
                              }
                            }
                          }
                          ...
                          // Verificar stock
                          if (stockActual === 0) {
                            toast.error(`⚠️ Sin stock de "${nombreProducto}". Agrega stock en Inventario.`);
                            return;
                          }
                          
                          if (stockActual < cantidad) {
                            toast.error(`Stock insuficiente. Solo hay ${stockActual} disponibles de "${nombreProducto}"`);
                            return;
                          }
                          ...
                          // Descontar del inventario usando la clave correcta
                          setPlatillosInventario(prev => {
                            const nuevoStock = {
                              ...prev,
                              [claveInventario]: (prev[claveInventario] || 0) - cantidad
                            };
                            localStorage.setItem('platillosInventario', JSON.stringify(nuevoStock));
                            return nuevoStock;
                          });
                          
                          toast.success(`✅ ${cantidad}x ${nombreProducto} agregado. Stock restante: ${stockActual - cantidad}`);
                        } else {
