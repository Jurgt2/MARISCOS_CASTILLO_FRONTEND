...
// --- Order store actions ---
const {
  ordersByTable, // Nuevo: objeto { [mesaId]: order }
  addItemToTable, // Nuevo: función para agregar item a mesa específica
  generateTicketForTable, // Nuevo: función para generar ticket por mesa
  // ...otros métodos...
} = useOrderStore();
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start justify-center">
            ...
            <div className="w-full lg:w-1/3 flex flex-col items-center">
                ...
                <div className="card-body flex flex-col items-center">
                  ...
                  {selectedTables.length > 0 && (
                    <div className="mt-6 px-6 py-3 rounded-xl bg-white text-primary font-bold text-base shadow text-center border-4 border-primary">
                      Mesas seleccionadas: <span className="text-primary">{selectedTables.map(t => t.number).join(', ')}</span>
                    </div>
                  )}
                </div>
              </div>
              <div className="w-full">
                ...
                {selectedTables.length === 1 && ordersByTable[selectedTables[0].id] && (
                  <OrderSummary order={ordersByTable[selectedTables[0].id]} />
                )}
              </div>
                ...
                <div className="card-castillo">
                  <div className="card-body">
                    <button
                      ...
                      type="button"
                      onClick={() => generateTicketForTable(selectedTables[0].id)}
                    ...
                    {ordersByTable[selectedTables[0]?.id]?.ticket && (
                      <div className="mt-8 card-castillo p-6 bg-white shadow-xl">
                        <h3 className="text-xl font-bold text-warning mb-4">Ticket de la mesa {selectedTables[0].number}</h3>
                        <table className="w-full mb-4">
                          ...
                          <tbody>
                            {ordersByTable[selectedTables[0].id].items.map((item, idx) => (
                        ...
                        <div className="text-right text-2xl font-bold text-success mb-2">
                          Total: ${ordersByTable[selectedTables[0].id].total.toFixed(2)}
                        </div>
                      </div>
                    )}
                  </div>
                ...
                >
                  ...
                  {showMenu && (
                    <ProductGridLite
                      ...
                      onAddProduct={(product, cantidad, notas) => {
                        if (selectedTables.length === 1) {
                          addItemToTable(selectedTables[0].id, product, cantidad, notas);
                        } else {
