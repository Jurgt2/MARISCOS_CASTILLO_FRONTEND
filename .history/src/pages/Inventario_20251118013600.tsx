...
export default function Inventario() {
  ...
  const handlePesaje = () => {
    const productoId = prompt('Ingrese el ID del producto para actualizar el peso:');
    const producto = productos.find((p) => p.id === parseInt(productoId));

    if (producto) {
      const nuevoPeso = parseFloat(prompt(`Ingrese el nuevo peso para ${producto.nombre} (kg):`));
      if (nuevoPeso > 0) {
