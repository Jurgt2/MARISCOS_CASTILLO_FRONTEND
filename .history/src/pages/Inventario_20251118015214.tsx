...
export default function Inventario() {
  ...
  const [productos, setProductos] = useState([
    { id: 'FQR59', nombre: 'Camarón U15', categoria: 'PROTEINA', cantidad: 25, caducidad: '2025-12-01', proveedor: 'Mariscos del Golfo', estado: 'ÓPTIMO' },
    { id: 'NID92', nombre: 'Camarón U21', categoria: 'PROTEINA', cantidad: 30, caducidad: '2025-12-05', proveedor: 'Mariscos del Golfo', estado: 'ÓPTIMO' },
    { id: 'AEJ80', nombre: 'Pulpo Español', categoria: 'PROTEINA', cantidad: 15, caducidad: '2025-12-10', proveedor: 'Mariscos del Golfo', estado: 'ÓPTIMO' },
    { id: 'MIE57', nombre: 'Filete de Robalo', categoria: 'PROTEINA', cantidad: 20, caducidad: '2025-12-15', proveedor: 'Pescados Frescos', estado: 'ÓPTIMO' },
    { id: 'BNJ39', nombre: 'Mojarra', categoria: 'PROTEINA', cantidad: 25, caducidad: '2025-12-20', proveedor: 'Pescados Frescos', estado: 'ÓPTIMO' },
    { id: 'URV39', nombre: 'Huachinango', categoria: 'PROTEINA', cantidad: 3, caducidad: '2025-12-25', proveedor: 'Pescados Frescos', estado: 'BAJO' },
    { id: 'LYZ27', nombre: 'Jaiba', categoria: 'PROTEINA', cantidad: 10, caducidad: '2025-12-30', proveedor: 'Mariscos del Golfo', estado: 'ÓPTIMO' },
    { id: 'IYQ22', nombre: 'Ostiones', categoria: 'PROTEINA', cantidad: 8, caducidad: '2025-12-30', proveedor: 'Mariscos del Golfo', estado: 'ÓPTIMO' },
  ]);
  ...
  const handleRecepcion = () => {
    const productoIdInput = prompt('Ingrese el ID del producto para actualizar:');
    const productoId = productoIdInput || ''; // Asegurar que sea una cadena
    const producto = productos.find((p) => p.id === productoId);

    if (producto) {
      const cantidadInput = prompt(`Ingrese la cantidad recibida para ${producto.nombre}:`);
      const cantidad = cantidadInput ? parseFloat(cantidadInput) : 0;
      const caducidad = prompt(`Ingrese la nueva fecha de caducidad para ${producto.nombre} (YYYY-MM-DD):`) || '';

      if (cantidad > 0 && caducidad) {
        setProductos((prevProductos) =>
          prevProductos.map((p) =>
            p.id === producto.id ? { ...p, cantidad, caducidad } : p
          )
        );
  ...
  const handlePesaje = () => {
    const productoIdInput = prompt('Ingrese el ID del producto para actualizar el peso:');
    const productoId = productoIdInput || ''; // Asegurar que sea una cadena
    const producto = productos.find((p) => p.id === productoId);

    if (producto) {
      const nuevoPesoInput = prompt(`Ingrese el nuevo peso para ${producto.nombre} (kg):`);
      const nuevoPeso = nuevoPesoInput ? parseFloat(nuevoPesoInput) : 0;
      if (nuevoPeso > 0) {
        setProductos((prevProductos) =>
          prevProductos.map((p) =>
            p.id === producto.id ? { ...p, cantidad: nuevoPeso } : p
          )
        );
        alert(`Peso actualizado para ${producto.nombre}: ${nuevoPeso.toFixed(2)} kg`);
      } else {
