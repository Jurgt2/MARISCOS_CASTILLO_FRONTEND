...
interface Ingrediente {
  ...
  estado?: string; // Nueva propiedad opcional
  caducidad?: string; // Nueva propiedad opcional
}
...
export default function Inventario() {
  ...
  const handleRecepcion = () => {
    ...
    if (producto) {
      ...
      const unidad = prompt(`Ingrese la unidad (K para gramaje, L para líquido):`) || '';

      if (cantidad > 0 && caducidad && (unidad === 'K' || unidad === 'L')) {
        setProductos((prevProductos) =>
          prevProductos.map((p) =>
            p.id === producto.id ? { ...p, cantidad, caducidad, unidad } : p
          )
        );
