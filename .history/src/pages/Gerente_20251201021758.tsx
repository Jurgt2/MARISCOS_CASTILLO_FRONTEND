            ...
            <div className="w-full lg:w-1/3 flex flex-col items-center">
              <div className="card-castillo w-full mb-6">
                <div className="card-body flex flex-col items-center">
                  <h2 className="card-title text-primary mb-4 text-center text-2xl">Selecciona una mesa</h2>
                  {loadingTables ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-2 gap-6 w-full">
                      {tables.map((table) => (
                        <div
                          ...
                          className={`transition-all duration-200 cursor-pointer rounded-2xl shadow-lg p-6 flex flex-col items-center justify-center border-2 ${currentTable?.id === table.id ? 'border-primary bg-primary/10 scale-105' : 'border-gray-200 bg-white hover:bg-primary/5'}`}
                        >
                          <div className="text-5xl mb-2">{table.capacity <= 2 ? '👥' : table.capacity <= 4 ? '👨‍👩‍👦‍👦' : '👨‍👩‍👧‍👦👨‍👩‍👧‍👦'}</div>
                          <div className="text-2xl font-bold mb-1 text-primary">Mesa {table.number}</div>
                          <div className="text-sm text-gray-600 mb-2">{table.capacity} personas</div>
                          <div className={`mt-2 text-xs font-semibold uppercase tracking-wide ${table.status === 'DISPONIBLE' ? 'text-success' : table.status === 'OCUPADA' ? 'text-error' : 'text-warning'}`}>{table.status.replace('_', ' ')}</div>
                        </div>
                      ))}
                    </div>
                  )}
                  {currentTable && (
                    <div className="mt-6 px-6 py-3 rounded-xl bg-primary text-white font-bold text-xl shadow text-center border-4 border-success">
                      <span className="mr-2">🍽️</span>Mesa seleccionada: <span className="text-success">{currentTable.number}</span>
                    </div>
                  )}
                </div>
              </div>
