...
const Cocina = () => {
  ...
  // Estado para trackear items listos: {orderId: {itemIndex: boolean}}
  const [itemsListos, setItemsListos] = useState<Record<string, Record<number, boolean>>>({})
  ...
  const handleCompleteOrder = async (orderId: string) => {
    try {
      ...
      // Limpiar items listos de esta orden
      setItemsListos(prev => {
        const newState = { ...prev }
  ...
  const toggleItemListo = (orderId: string, itemIndex: number) => {
    setItemsListos(prev => {
      ...
      return {
        ...
        [orderId]: {
          ...orderItems,
          [itemIndex]: !orderItems[itemIndex]
        }
      }
    })
  }

  const isItemListo = (orderId: string, itemIndex: number) => {
    return itemsListos[orderId]?.[itemIndex] || false
  }

  const getTodosItemsListos = (orderId: string, totalItems: number) => {
    const orderItems = itemsListos[orderId] || {}
    let listos = 0
    for (let i = 0; i < totalItems; i++) {
      if (orderItems[i]) listos++
    }
    return { listos, total: totalItems }
  }
              ...
              >
                <div className="card-body">
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                      ...
                      {order.status === 'EN_PREPARACION' && (() => {
                        const { listos, total } = getTodosItemsListos(order.id!, order.items.length)
                        return (
                          <div className="mt-3">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-sm font-bold">Progreso:</span>
                              <span className="text-lg font-bold text-green-400">{listos}/{total}</span>
                            </div>
                            <progress 
                  ...
                  <div className="space-y-4 mb-4 max-h-[60vh] overflow-y-auto">
                    {Object.entries(agruparPorCategoria(order.items)).map(([categoria, items]) => {
                      // Calcular índices globales de los items
                      let globalIndex = 0
                      for (let i = 0; i < order.items.indexOf(items[0]); i++) {
                        globalIndex = i
                      }
                      
                      return (
                        <div key={categoria} className="bg-black/40 p-4 rounded-lg border-2 border-white/20">
                          {/* Título de categoría */}
                          <h4 className="text-2xl font-bold mb-3 text-yellow-300 border-b-2 border-yellow-300/50 pb-2">
                            {categoria === 'CARNES' && '🥩'}
                            {categoria === 'NIÑOS' && '👶'}
                            {categoria === 'POSTRES' && '🍰'}
                            {categoria === 'CAMARONES' && '🦐'}
                            ...
                            {categoria === 'SOPAS' && '🍲'}
                            {categoria === 'CHILPACHOLES' && '🌶️'}
                            {categoria === 'CALDO LARGO' && '🥘'}
                            {categoria === 'PESCADOS' && '🐟'}
                            {categoria === 'HUEVA DE PESCADO' && '🥚'}
                            {categoria === 'FILETES' && '🐠'}
                            {categoria === 'PULPOS' && '🐙'}
                            {categoria === 'COCTELES' && '🍹'}
                            {categoria === 'ENTRADAS' && '🍤'}
                          ...
                          <div className="space-y-2">
                            {items.map((item) => {
                              ...
                              const itemListo = isItemListo(order.id!, itemIndex)
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-start gap-3">
                                    <div className={`text-3xl font-bold min-w-[50px] ${
                                      itemListo ? 'text-green-300' : 'text-green-400'
                                    }`}>
                                      {item.quantity}x
                                    </div>
                                    <div className="flex-1">
                                      <div className={`text-xl font-bold ${itemListo ? 'line-through opacity-60' : ''}`}>
                                        {item.product.name}
                                      </div>
                                      ...
                                      {item.notes && (
                                        <div className="bg-warning/20 p-2 rounded mt-2 text-sm border border-warning">
                                    ...
                                    <button
                                      onClick={() => toggleItemListo(order.id!, itemIndex)}
                    ...
                    {order.status === 'PENDIENTE' ? (
                      ...
                      return (
                        <button
                          ...
                          className={`btn btn-lg w-full text-xl font-bold ${
                            todosListos 
                              ? 'btn-success' 
