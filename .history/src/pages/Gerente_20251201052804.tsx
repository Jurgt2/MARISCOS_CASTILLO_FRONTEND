                ...
                <div className="card-castillo">
                  <div className="card-body">
                    ...
                    {ordersByTable[selectedTables[0]]?.ticket && (
                      <div className="mt-8 card-castillo p-4 bg-white shadow-2xl rounded-2xl border-2 border-yellow-400 max-w-xs mx-auto">
                        <h3 className="text-2xl font-bold text-yellow-600 mb-6 text-center">Ticket de la mesa {tables.find(t => t.id === selectedTables[0])?.number}</h3>
                        <table className="w-full mb-6 rounded overflow-hidden">
                          <thead>
                            <tr className="bg-yellow-100 text-yellow-800">
                              <th className="text-left py-0.5 px-0.5 font-semibold text-xs">Platillo</th>
                              <th className="text-center py-0.5 px-0.5 font-semibold text-xs">Cantidad</th>
                              <th className="text-center py-0.5 px-0.5 font-semibold text-xs">Precio unitario</th>
                              <th className="text-center py-0.5 px-0.5 font-semibold text-xs">Notas</th>
                              <th className="text-right py-0.5 px-0.5 font-semibold text-xs">Subtotal</th>
                            </tr>
                          </thead>
                          <tbody>
                            {ordersByTable[selectedTables[0]].items.map((item, idx) => (
                              <tr key={idx} className="border-b hover:bg-yellow-50">
                                <td className="py-2 px-2 font-bold text-gray-700">{item.product.name}</td>
                                <td className="py-2 px-2 text-center text-gray-600">{item.quantity}</td>
                                <td className="py-2 px-2 text-center text-green-700 font-bold">${item.product.price.toFixed(2)}</td>
                                <td className="py-2 px-2 text-center text-xs text-gray-500">{item.notes || '-'}</td>
                                <td className="py-2 px-2 text-right font-bold text-green-700">${item.subtotal.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                        <div className="flex items-center justify-between mt-4 mb-2">
                          ...
                          <span className="text-2xl font-extrabold text-green-600">${ordersByTable[selectedTables[0]].total.toFixed(2)}</span>
                        </div>
