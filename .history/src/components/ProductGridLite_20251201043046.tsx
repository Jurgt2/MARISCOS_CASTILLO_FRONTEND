...
import { menuCompleto } from '../data/menu';
import { Product } from '../store/orderStore';
...
interface ProductGridLiteProps {
  ...
  onAddProduct?: (product: Product, cantidad: number, notas: string, modifiers: string[]) => void;
}
...
export function ProductGridLite({ productos, selectedTables }: ProductGridLiteProps) {
  ...
  const safeOnAddProduct = typeof onAddProduct === 'function'
    ...
    : (product: Product, cantidad: number, notas: string, modifiers: string[]) => {
        if (currentTable) {
          addItemToTable(currentTable, product, cantidad, notas, modifiers);
          toast.success(`${product.name || product.nombre} agregado`);
        } else {
