...
export function ProductGridLite({ productos, selectedTables }: ProductGridLiteProps) {
  ...
  // Añadir onAddProduct al destructuring de props
  const safeOnAddProduct = typeof arguments[0].onAddProduct === 'function'
    ? arguments[0].onAddProduct
  ...
  <h2 className="text-2xl font-bold mb-4 ml-40">Menú Completo</h2>
      <input
        type="text"
        className="input input-bordered mb-4 w-64 ml-auto block"
        value={search}
      ...
      <div className="space-y-10">
        {categorias.map(categoria => (
          <div key={categoria}>
            <h3 className="text-2xl font-bold text-primary mb-4 uppercase tracking-wide ml-40">{categoria}</h3>
            <div className="w-full overflow-x-auto pb-4 ml-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-10 gap-y-10 justify-items-center items-center min-w-max">
                {lista.filter(p => p.categoria === categoria).map(product => (
                  <div key={product.id} className="card bg-white shadow-2xl rounded-2xl border-2 border-gray-300 p-3 flex flex-col items-center justify-between min-h-[140px] min-w-[170px] max-w-[170px] mx-2 my-2">
                    <div className="font-extrabold text-base text-primary mb-1 text-center uppercase leading-tight break-words whitespace-normal">
                      {product.nombre}
                    </div>
                    <div className="text-xs text-gray-700 mb-1 text-center break-words whitespace-normal">
                      {product.descripcion}
                    </div>
                    <div className="font-bold text-base text-success mb-1">${product.precio.toFixed(2)}</div>
                    <button className="btn btn-primary w-full mt-1 font-bold text-sm py-1 rounded-xl" onClick={() => handleAdd(product)}>
                      Agregar
                    </button>
                  </div>
                ))}
            ...
            {selectedProduct.variantes && selectedProduct.variantes.length > 0 && (
              <div className="mb-3">
                ...
                <select
                  ...
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSelectedVariante(e.target.value)}
                >
