        ...
        {categorias.map(categoria => (
          <div key={categoria}>
            ...
            <div className="w-full overflow-x-auto pb-4 ml-8">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-16 gap-y-16 justify-items-center min-w-max">
                {lista.filter(p => p.categoria === categoria).map(product => (
                  <div key={product.id} className="card bg-white shadow-2xl rounded-2xl border-2 border-gray-300 p-4 flex flex-col items-center justify-between min-h-[180px] min-w-[220px] max-w-[220px] max-h-[220px] aspect-square mx-2 my-2">
                    <div className="font-extrabold text-lg text-primary mb-2 text-center uppercase leading-tight break-words whitespace-normal">
                      {product.nombre}
                    </div>
                    <div className="text-base text-gray-700 mb-2 text-center break-words whitespace-normal">
                      {product.descripcion}
                    </div>
                    <div className="font-bold text-2xl text-success mb-2">${product.precio.toFixed(2)}</div>
                    <button className="btn btn-primary w-full mt-2 font-bold text-lg py-3 rounded-xl" onClick={() => handleAdd(product)}>
                      Agregar
                    </button>
                  </div>
                ))}
